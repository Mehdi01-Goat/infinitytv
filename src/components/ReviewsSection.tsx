    ...
    <section className="py-20 border-t border-border">
      <div className="container">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-10">
          <p className="text-primary font-semibold text-sm uppercase tracking-widest mb-3">Customer Reviews</p>
          <h2 className="font-heading text-2xl sm:text-3xl md:text-4xl font-bold mb-3">What Our <span className="text-gradient">Customers Say</span></h2>
          <p className="text-muted-foreground max-w-xl mx-auto mb-6">Trusted by thousands of satisfied viewers worldwide.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {reviews.map((review, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              className="rounded-xl border border-border glass-card p-5 flex flex-col gap-3 hover:border-glow transition-all duration-300">
              ...
              <div className="flex items-center justify-between pt-1 border-t border-border">
                ...
                <div className="flex items-center gap-1 text-[10px] text-emerald-600 font-medium">
                  <CheckCircle size={11} /><span>Verified</span>
                </div>
              </div>
